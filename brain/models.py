...
# --- Proposal Models (Helferlein System) ---

class ProposalStatus(Enum):
    """Status of a helferlein proposal."""
    PENDING = "pending"
    APPROVED = "approved"
    REJECTED = "rejected"
    EXECUTING = "executing"
...
class ProposalActionType(Enum):
    """What kind of action the proposal suggests."""
    CHANGE = "change"       # Code/file changes (needs Claude Code CLI)
    APPLY = "apply"         # Apply for a job/project (needs review)
    DRAFT = "draft"         # Content draft (LinkedIn post, email reply)
    NOTIFY = "notify"       # Information only (no action needed)
    FIX = "fix"             # Bug fix / test failure (needs Claude Code CLI)


@dataclass
class Proposal:
    """A proposal generated by a helferlein for user approval.

    Helferlein research/crawl/check overnight and create proposals.
    User reviews them in the dashboard and approves/rejects.
    Approved proposals with a prepared_prompt get executed via Claude Code CLI.
    """
    id: str
    helferlein: str                           # "akquise" | "content" | "admin" | "projekt"
    title: str                                # Short description
    description: str                          # What was found, why it matters
    priority: str = "normal"                  # "high" | "normal" | "low"
    action_type: ProposalActionType = ProposalActionType.NOTIFY
    evidence: list[str] = field(default_factory=list)  # Paths to screenshots, data files
    prepared_prompt: str = ""                 # Claude Code CLI prompt (ready to execute)
    prepared_plan: str = ""                   # Step-by-step what Claude should do
    project: str | None = None                # Associated project name
    project_path: str | None = None           # Resolved project path
    status: ProposalStatus = ProposalStatus.PENDING
    user_comment: str | None = None           # User's note on approve/reject
    session_id: str | None = None             # Claude Code session ID if executing
    created_at: datetime = field(default_factory=datetime.now)
